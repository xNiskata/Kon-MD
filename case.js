...
client.sendText(m.chat, 'List Online:\n\n' + online.map(v => '${sp} @' + v.replace(/@.+/, '')).join`\n`, m, { mentions: online })
 ...
 break
case prefix+'sticker': case prefix+'s': case prefix+'stickergif': case prefix+'sgif': case prefix+'stiker' :{
...
case prefix+'fb': case prefix+'facebook': case prefix+'igmp4': case prefix+'ytvideo': case prefix+'ytmp4': {
addCountCmd(`#${command.slice(1)}`, sender, _cmd)
if (!args[0]) throw 'Linknya mana?'
m.reply(mess.wait)
...
└	◦ ${prefix}setexif
`
var button = [{ buttonId: `${prefix}dashboard`, buttonText: { displayText: `🏘️ Dashboard` }, type: 1 }, { buttonId: `${prefix}owner`, buttonText: { displayText: `👤 Owner` }, type: 1 }]
